...
//  GreenThumbTracker
//
//  Created by Toby Buckmaster on 4/15/25.
//
...
struct GrowthChartView: View {
    ...
    private var dateLabelFormatter: DateFormatter {
        ...
        var filteredRecords: [GrowthRecord] {
            let now = Date()
        ...
        private var ChartContainerView: some View {
            VStack(spacing: 8) {
                Text("Growth Over \(selectedRange.rawValue)")
                    .font(.headline)
                    ...
                    HStack {
                        Button(action: {
                            ...
                            if let image = renderer.uiImage {
                                let saver = PhotoSaveHelper { success in
                                    saveConfirmationMessage = success ? "Photo saved!" : "Failed to save."
                        ...
                        Button(action: {
                            ...
                            if let image = renderer.uiImage {
                                ...
                                printInfo.jobName = "Growth Chart"
