...
struct SoilMoistureChartView: View {
    let soilMoistureRecords: [SoilMoistureRecord]
       ...
       private var filteredRecords: [SoilMoistureRecord] {
           let now = Date()
           let calendar = Calendar.current
           return soilMoistureRecords.filter {
       ...
       private var ChartContainerView: some View {
           VStack(spacing: 8) {
               Text("Soil Moisture Over \(selectedRange.rawValue)")
                   .font(.headline)
               ...
               Chart {
                   ForEach(filteredRecords.sorted(by: { $0.date < $1.date })) { record in
                       if let date = isoFormatter.date(from: record.date) {
                           LineMark(
                               x: .value("Date", date),
                               y: .value("Soil Moisture", record.soil_moisture)
                           )
                   ...
                   HStack {
                       ...
                       Button(action: {
                           ...
                           if let image = renderer.uiImage {
                               ...
                               printInfo.jobName = "Soil Moisture Chart"
